...
crossSectionKeyPoints = [
    [[[(0, 0), (100, -1.27)], [(10, -1.27), (10+1.27+5, -2*1.27)], [(90-1.27-5, -1.27), (90, -2*1.27)], [(10, -1.27*2), (10+1.27, -1.27-75)], [(90-1.27, -1.27*2), (90, -1.27-75)], [(10+1.27, -75), (90-1.27, -1.27-75)]], 0],
    [[[(0, 0), (100, -1.27)], [(10, -1.27), (10+1.27+5, -2*1.27)], [(90-1.27-5, -1.27), (90, -2*1.27)], [(10, -1.27*2), (10+1.27, -1.27-75)], [(90-1.27, -1.27*2), (90, -1.27-75)], [(10+1.27, -75), (90-1.27, -1.27-75)]], 1200]
] # [[cross section, position]] ...]

glueKeyPoints = [
    [[-1.27, 10, 0], [-1.27, 10, 1200]], # First Glue Tab
    # 2nd Glue Tab
    # ...
...
type1FailureKeyPoints = [
    ...
    [[-1.27-75, 1.27, 80, 0], [-1.27-75, 1.27, 80, 1200]] # 2nd Place
    # ...
...
# Since the code calculates the factors of safety slightly differently, some parts have been copied
# here to that their outputs can be printed to the screen
# m = calculateMaxLoad()
# print("Max Possible Load: " + f"{m:.4g}")
crossSection = getCrossSection(0)
yAvg = getYAvg(crossSection)
print("Centroidal Axis Location: "+f"{yAvg:.8g}"+" mm from the top")

secondMomentOfArea = get2ndMomentOfArea(crossSection)
print("2nd Moment of Area: "+f"{secondMomentOfArea:.8g}"+" mm^4")

maxShear = max([abs(i) for i in calculateMaxSFD(400)])

# Gets the Q at the centroidal axis, this is to show intermediate calculations
Q = 0
for rect in crossSection:
    if rect[1][1] >= yAvg and rect[0][1] >= yAvg:
        Q += abs((rect[1][0]-rect[0][0])*(rect[1][1]-rect[0][1]))*((rect[1][1]+rect[0][1])/2-yAvg)
    elif rect[1][1] <= yAvg and rect[0][1] >= yAvg:
        Q += abs((rect[1][0]-rect[0][0])*(rect[0][1]-yAvg))*((rect[0][1]+yAvg)/2-yAvg)

# Gets the width at the centroidal axis
width = 0
for rect in crossSection:
    if rect[1][1] <= yAvg and rect[0][1] >= yAvg:
        width += rect[1][0]-rect[0][0]

print("Maximum Shear Force: "+f"{maxShear:.8g}")
print("Q at Centroidal Axis: ", f"{Q:.8g}")
print("Maximum Shear Stress: "+f"{maxShear*Q/secondMomentOfArea/width:.8g}")

# Gets the Q at the glue joint, this is to show intermediate calculations
Q = 0
for rect in crossSection:
    if rect[1][1] >= glueKeyPoints[0][0][0] and rect[0][1] >= glueKeyPoints[0][0][0]:
        Q += abs((rect[1][0]-rect[0][0])*(rect[1][1]-rect[0][1]))*((rect[1][1]+rect[0][1])/2-yAvg)
    elif rect[1][1] <= glueKeyPoints[0][0][0] and rect[0][1] >= glueKeyPoints[0][0][0]:
        Q += abs((rect[1][0]-rect[0][0])*(rect[0][1]-glueKeyPoints[0][0][0]))*((rect[0][1]+glueKeyPoints[0][0][0])/2-yAvg)
print("Q at Glue Tab: ", f"{Q:.8g}")
print("Shear Stress at Glue Tab: ", f"{maxShear*Q/secondMomentOfArea/glueKeyPoints[0][0][1]:.4g}")

# Gets the moments and the compression
maxMoment = max([abs(i) for i in calculateMaxBMD(400)])
print("Maximum Bending Moment: "+f"{maxMoment:.8g}")
maxYTension = abs(yAvg-min([rect[1][1] for rect in crossSection]))
print("Maximum Tensile Stress: "+f"{maxMoment*maxYTension/secondMomentOfArea:.8g}")
maxYCompression = abs(yAvg-max([rect[0][1] for rect in crossSection]))
print("Maximum Compressive Stress: "+f"{maxMoment*maxYCompression/secondMomentOfArea:.8g}")

# Gets the max allowable stress for case 1
position = 984
rectIndex = 0
y, t, b = interpolateType12Failure(type1FailureKeyPoints, position)[rectIndex]
crossSection = getCrossSection(position)
...
yAvg = getYAvg(crossSection)
y = y-yAvg
stressAllowable = ((4*pi**2*matboardYoungsModulus)/(12*(1-matboardPoissons**2))*(t/b)**2)
loadStress = maxMoment*y/I
print("Critical Stress For Type 1 Failure: ", f"{stressAllowable:.8g}")
print("Load Stress For Type 1 Failure: ", f"{loadStress:.8g}")

# Gets the max allowable stress for case 2
y, t, b = interpolateType12Failure(type2FailureKeyPoints, position)[rectIndex]
crossSection = getCrossSection(position)
I = get2ndMomentOfArea(crossSection)
yAvg = getYAvg(crossSection)
y = y-yAvg
stressAllowable = ((0.425*pi**2*matboardYoungsModulus)/(12*(1-matboardPoissons**2))*(t/b)**2)
loadStress = maxMoment*y/I
print("Critical Stress For Type 2 Failure: ", f"{stressAllowable:.8g}")
print("Load Stress For Type 2 Failure: ", f"{loadStress:.8g}")

# Gets the max allowable stress for case 3
y, t = interpolateType3Failure(type3FailureKeyPoints, position)[rectIndex]
crossSection = getCrossSection(position)
I = get2ndMomentOfArea(crossSection)
yAvg = getYAvg(crossSection)
y = y-yAvg
stressAllowable = ((6*pi**2*matboardYoungsModulus)/(12*(1-matboardPoissons**2))*(t/y)**2)
loadStress = maxMoment*y/I
print("Critical Stress For Type 3 Failure: ", f"{stressAllowable:.8g}")
print("Load Stress For Type 3 Failure: ", f"{loadStress:.8g}")

# Gets the max allowable stress for case 4
# Finds the closest diaphragms
lowerBound = [x for x in diaphragmPositions if x < position]
upperBound = [x for x in diaphragmPositions if x >= position]
if len(lowerBound) == 0:
    lowerBound = 0
...
smallestShear = float("inf")
for position2 in list(np.linspace(lowerBound, upperBound, 1000))+[position]:
    shear = (5*pi**2*matboardYoungsModulus)/(12*(1-matboardPoissons**2))*((t/a)**2+(t/h)**2)
    if shear < smallestShear:
        smallestShear = shear
stressAllowable = smallestShear
Q = 0
for rect in crossSection:
    if rect[1][1] >= yAvg and rect[0][1] >= yAvg:
        Q += abs((rect[1][0]-rect[0][0])*(rect[1][1]-rect[0][1]))*((rect[1][1]+rect[0][1])/2-yAvg)
        ...
        Q += abs((rect[1][0]-rect[0][0])*(rect[0][1]-yAvg))*((rect[0][1]+yAvg)/2-yAvg)
loadStress = maxShear*Q/I/width
print("Critical Stress For Type 4 Failure: ", f"{stressAllowable:.8g}")
print("Load Stress For Type 4 Failure: ", f"{loadStress:.8g}")

# Gets the safety factors
sf = calculateSafetyFactors(400)
for i in sf:
    print("Failure Mode:", i, "   FOS: " + f"{sf[i]:.8g}")
