...
def plotSFD(trainWeight):
    ...
    ax1.set_ylim(top=10*max(maxSFD), bottom=0)
...
def plotBMD(trainWeight):
    ...
    ax1.set_ylim(top=10*max(BMD), bottom=0)
...
plotBridge()
for i in crossSectionKeyPoints[:len(crossSectionKeyPoints)//2]:
    plotCrossSectionAtPosition(i[1])
