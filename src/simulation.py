...
_PARAMS_PATH = Path(__file__).resolve().parents[1] / "config" / "design.py"
_PARAMS_SPEC = importlib.util.spec_from_file_location("params", _PARAMS_PATH)

params = importlib.util.module_from_spec(_PARAMS_SPEC)
_PARAMS_SPEC.loader.exec_module(params)

globals().update(
    {name: getattr(params, name) for name in dir(params) if not name.startswith('_')}
)
